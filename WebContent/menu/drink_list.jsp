...
<%@ page language="java" contentType="text/html; charset=UTF-8"
 ...
 <%   
	...
	List<CategoryDTO> categoryType = categoryDao.categoryTypeImport("drink");
	List<MenuDTO> menuId = menuDao.menuCategoryIdImport(null);
	List<MenuDTO> newmenu = menuDao.newMenuImport("drink");
...
<head>
<meta charset="UTF-8">
<title>음료 | Ryanbucks Korea</title>
...
<link href="/RYANBUCKS_v1.0/common/css/jQuerymyCustomScrollbar.css" rel="stylesheet">
<link href="/RYANBUCKS_v1.0/common/css/ezmark.css" rel="stylesheet">
...
<div id="container">
	...
	<div class="sub_tit_wrap">
		<div class="sub_tit_inner">
			<h2>
				<img src="/RYANBUCKS_v1.0/non_co_img/sub_title/drink_tit.jpg" alt="음료">
			</h2>
			<ul class="smap">
				<li>
				...
				<li>
					<a href="javascript:void(0)" class="this">음료</a>
				</li>
			</ul>
		</div>	
	</div>
	...
	<div class="content">
		...
		<div class="product_kind_wrap">
			...
			<div class="product_toggle_wrap" display="block">
				<dl class="product_kind_tab">
					<dt class="dt1">
						<a href="javascript:void(0)" id="categoryTab" class="tab selected" role="button" title="카테고리별 음료선택">카테고리</a>
					</dt>
					<dd id="panel" style="display:block">
						<div class="product_select_wrap">
							<form method="post">
								<fieldset>
									<legend class="hid">음료 카테고리 별 분류보기</legend>
					...
					<dt class="dt2">
						<a href="javascript:void(0)" id="themeTab" title="테마별 음료보기" class="tab">신메뉴</a>
					</dt>
					<dd class="panel" style="display:none">
						<ul class="cate_list service_bn">
							<li>
								<a href="javascript:void(0)" role="button">
									<img src="/RYANBUCKS_v1.0/non_co_img/menu/seasonmenu.jpg" alt="시즌 프로모션" data-sbseq="W0000461" style="width: 335px; height: 90px">
								</a>
							</li>
						</ul>
					</dd>
		...
		<!-- product_result_wrap product_result_wrap01, 음료 리스트(카테고리 별) -->
						...
						<div class="product_list">
							<dl>
								<% 
									for(CategoryDTO categoryDto : categoryType) {
								%>					
								<dt class="<%= categoryDto.getId() %> coffee" style="display:;">
									<a href="javascript:void(0);"><%= categoryDto.getName() %></a>
								</dt>
								
								<dd class="<%= categoryDto.getId() %> coffee" style="display:;">
									<ul>
												   ...
												   <% for (MenuImageDTO menuImgDto : menuImgList) {
												    ...
												    %>
										             ...
										             <div class="image_item">
										                 <a href="drink_view.jsp?menu_no=<%= menuDto.getNo() %>" class="goDrinkView">
		...
		<!-- product_result_wrap product_result_wrap02, 음료 리스트(신메뉴) -->
	...
	<script>
		 $(document).ready(function() {
		        $("#categoryTab").on("click",function() {
		            ...
		            $("#panel").css("display", "block");			// 카테고리 음료 종류
		            $(".panel").css("display", "none");				// 신메뉴 카테고리 
		            $("#category_list").css("display", "block");	// 카테고리 음료 리스트
		            $("#new_list").css("display", "none");			// 신메뉴 음료 리스트
		        });
		        ...
		        $("#themeTab").on("click",function() {
		            ...
		            // 형제 요소 선택
		            $("#panel").css("display", "none");				// 카테고리 음료 종류
		            $(".panel").css("display", "block");			// 신메뉴 카테고리 
		            $("#category_list").css("display", "none");		// 카테고리 음료 리스트
		            $("#new_list").css("display", "block");			// 신메뉴 음료 리스트
		        });	
		    });
	</script>
	    ...
	    if(this.checked) {
	        ...
	        for(let ind = 0; ind < checkboxes.length; ind++){
	         ...
	         if (checked && id == 'product_all') {		 
				$("dt.coffee").css("display", "block");
				$("dd.coffee").css("display", "block");
					
			} else {
				$("dt.coffee").css("display", "none");
				$("dd.coffee").css("display", "none");
			} 
